...
router.get(`/`, (req, res) => {
  db.getProjects()
    ...
    .catch(error => {
      console.error(error);
      res.status(500).json({ errorMessage: `Not able to retrieve projects` });
    });
});
...
router.get(`/:id/tasks`, validateId, (req, res) => {
  ...
  db.getProjectTasks(id)
    ...
    .catch(error => {
      console.error(error);
      res.status(500).json({ errorMessage: `Not able to retrieve tasks` });
    });
});
...
router.get(`/:id/resources`, validateId, (req, res) => {
  ...
  db.getProjectResources(id)
    ...
    .catch(error => {
      console.error(error);
      res.status(500).json({ errorMessage: `Not able to retrieve resources` });
    });
});
...
router.post(`/`, (req, res) => {
  ...
  if (projectData.name) {
    db.addProject(projectData)
      ...
      .catch(error => {
        console.error(error);
        res.status(500).json({ errorMessage: `Not able to add project` });
      });
  } else {
...
router.post(`/:id/task`, validateId, (req, res) => {
  ...
  if (taskData.project_id = id) {
    db.addTask(taskData, id)
      ...
      .catch(error => {
        console.error(error);
        res.status(500).json({ errorMessage: `Not able to add task` });
      });
  } else {
    res.status(400).json({ message: `The body must contain "description"` });
  }
});
  ...
  db.addResourceToProject(projectId, resourceId)
    .then(added => {
      if (added) {
        ...
        res
          .status(404)
          .json({ message: `Not able to locate resource ${resourceId}` });
      }
    })
    .catch(error => {
      console.error(error);
      res
        .status(500)
        .json({ errorMessage: `Not able to add resource to project` });
    });
});
